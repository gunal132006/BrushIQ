async function runSeeding(db) {
  console.log('Verifying initial systemic data...');

  try {
    const tipsCountRes = await db.query('SELECT COUNT(*)::int as count FROM tips');
    const count = (tipsCountRes && tipsCountRes.rows && tipsCountRes.rows.length > 0 && tipsCountRes.rows[0].count !== undefined)
      ? parseInt(tipsCountRes.rows[0].count, 10)
      : 0;
    ...
    if (count === 0) {
      ...
      console.log(`Seeded ${tipsList.length} systemic oral care tips.`);
    }
  } catch (err) {
    console.error('Error during data seeding check:', err.message);
  }


  console.log('Systemic data verification complete.');
}

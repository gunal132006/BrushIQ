...
import { User, Mail, Phone, Lock, AlertCircle, Sun, Moon, CheckCircle2 } from 'lucide-react';
...
const Register = () => {
  ...
  const features = [
    'AI-powered bristle wear detection',
    'Multi-member family profiles',
    'Smart replacement reminders',
    'Detailed scan history & reports',
    'Personalised hygiene tips',
  ];
  ...
  return (
    <div className={`min-h-screen w-full flex relative transition-colors duration-200 ${
      ...
      {/* LEFT — Hero Panel */}
      <div className="hidden lg:flex w-5/12 flex-col justify-between p-12 relative overflow-hidden bg-gradient-to-br from-secondary via-[#0fa896] to-primary">
        ...
        <div className="relative z-10">
          <h2 className="text-3xl font-black text-white leading-tight mb-3">
            Join thousands<br />
            caring for their<br />
            <span className="text-white/70">oral health smarter.</span>
          </h2>
          <p className="text-white/70 text-sm leading-relaxed mb-8 max-w-xs">
            Create your free account and start tracking your toothbrush health with AI precision.
          </p>
      ...
      <div className={`flex-1 flex flex-col items-center justify-center px-8 py-12 overflow-y-auto ${
        ...
        <div className="w-full max-w-md">
          ...
          <p className={`text-sm mb-8 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
            Get started with bristle wear analysis — it's free
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Full Name */}
            <div>
              <label htmlFor="fullName" className={`block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${darkMode ? 'text-slate-400' : 'text-slate-400'}`}>
                Full Name
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 rounded-xl border outline-none font-semibold text-sm transition-all focus:ring-2 focus:ring-primary/20 ${
                    darkMode ? 'bg-slate-950 border-slate-800 focus:border-primary text-white' : 'bg-slate-50 border-slate-200 focus:border-primary focus:bg-white text-slate-900'
                  }`}
                  disabled={loading}
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="email" className={`block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${darkMode ? 'text-slate-400' : 'text-slate-400'}`}>
                  Email
                </label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-3 py-3 rounded-xl border outline-none font-semibold text-sm transition-all focus:ring-2 focus:ring-primary/20 ${
                      darkMode ? 'bg-slate-950 border-slate-800 focus:border-primary text-white' : 'bg-slate-50 border-slate-200 focus:border-primary focus:bg-white text-slate-900'
                    }`}
                    disabled={loading}
                  />
              ...
              <div>
                <label htmlFor="phone" className={`block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${darkMode ? 'text-slate-400' : 'text-slate-400'}`}>
            ...
            {/* Password */}
            <div>
              <label htmlFor="password" className={`block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${darkMode ? 'text-slate-400' : 'text-slate-400'}`}>
                Password
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400">
